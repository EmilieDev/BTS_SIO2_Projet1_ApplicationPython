...
CREATE TABLE IF NOT EXISTS `etudes` (
  ...
  `descEtude` text,
  `idTypeEtu` int DEFAULT NULL,
  `idOrganisme` int DEFAULT NULL,
  `dateDebEtu` date DEFAULT NULL,
  `dateFinEtu` date DEFAULT NULL,
  `idMedResponsable` int DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idTypeEtu` (`idTypeEtu`),
  KEY `idOrganisme` (`idOrganisme`),
  KEY `idMedResponsable` (`idMedResponsable`)
...
INSERT INTO `etudes` (`id`, `nomEtu`, `descEtude`, `idTypeEtu`, `idOrganisme`, `dateDebEtu`, `dateFinEtu`, `idMedResponsable`) VALUES
(1, 'Étude Cancer Poumon 2020', 'Suivi de patients atteints de cancer du poumon', 2, 1, '2020-01-01', '2024-12-31', 1),
(2, 'Étude Cirrhose Lyon', 'Ancien essai (non utilisé car hors cancer, conservé pour cohérence historique)', 1, 2, '2019-05-15', '2023-12-31', 2),
(3, 'Essai Chirurgie Cancer Rectum', 'Évaluation des résultats post-opératoires en chirurgie rectum', 2, 1, '2022-01-01', '2026-12-31', 4),
(4, 'Protocole RecaRe', 'Essai multicentrique sur la prise en charge du cancer rectum : chirurgie + radiochimiothérapie', 2, 2, '2023-01-01', '2027-12-31', 4),
(5, 'Étude Cancer Sein', 'Essai clinique sur traitements innovants cancer du sein', 2, 3, '2021-01-01', '2025-12-31', 5);
...
INSERT INTO `medecins` (`id`, `nomSoignant`, `prenomSoignant`, `idSpecialite`, `idService`, `idRole`) VALUES
(1, 'Durand', 'Paul', 1, 1, 1),
...
CREATE TABLE IF NOT EXISTS `patientsmaladies` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_patient_maladie` (`idPatient`,`idMaladie`),
...
--
-- Structure de la table `typeetudes`
--

DROP TABLE IF EXISTS `typeetudes`;
CREATE TABLE IF NOT EXISTS `typeetudes` (
  `id` int NOT NULL AUTO_INCREMENT,
...
--
-- Déchargement des données de la table `typeetudes`
--

INSERT INTO `typeetudes` (`id`, `libelleEtu`) VALUES
